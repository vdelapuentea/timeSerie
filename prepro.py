...
def funcion(data, col_lugar, col_fecha):    
    index,X,Y,index_total,dic=[],[],[],[],{}
    for i in range(len(data[col_lugar].unique())):
        index.append(i)
        dic[i]=data[col_lugar].sort_values().unique()[i]
    ...
    split = train_test_split(dX,dY, test_size=0.25, random_state=42)
    (x_train, x_test, y_train, y_test) = split
    
    train_split = np.concatenate((x_train,y_train), axis=1)
    train_split2=np.concatenate((dX,dY), axis=1)
    scaler =  StandardScaler()   
    scaler.fit(train_split)
    X=scaler.transform(np.concatenate((dX,dY), axis=1))
    DX,DY,b=[],[],X.shape[1]
    for i in X:
        DX.append(np.array(i[:b-1]))
        DY.append(np.array(i[b-1:]))
    return np.array(ind),np.array(DX),np.array(DY), scaler, dic, np.concatenate((x_train,y_train), axis=1).std() , np.concatenate((x_test,y_test), axis=1).std()
